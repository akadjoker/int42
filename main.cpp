...
private:
	...
	string desc;
public:
	SyntaxError(int line, const string &msg): lineno(line), desc(msg){}
	void What()
    {
        cout << "\nError: at line (" << lineno<<") "<< desc << endl;
    }
};
    ...
    shared_ptr<Token> get_next_token()
    {
        while (current != '\0')
        {
            ...
            if (isDigit(current))
            {
                ...
                // return   new_token(TokenType::TK_NUMBER, current, 1);
            }else
            ...
            {
                isPanic = true;
              //  throw SyntaxError(line, "Unknow character ("+ std::to_string(current) +") At :" + std::to_string(cursor));
                cout<<"Lexer Error: Unknow character '"<<current<<"' Line: "<<line<<" ' Row "<<cursor<<endl;
              //  advance();
...
public:
    ...
    void eat(TokenType token_type)
    {
        ...
        if (current_token->type == token_type)
            ...
            throw SyntaxError(Lineno(), "Expected ("+tokeToString(token_type)+" Get ("+tokeToString(type)+")");
    }
    ...
    shared_ptr<AST> expr()
    {
        ...
        while (current_token->type == TokenType::TK_PLUS || current_token->type == TokenType::TK_MINUS)
        {
            ...
            {
                cout << "Praser Error: Invalid syntax" << endl;
            }
        }
...
public:
    ...
    double Execute(shared_ptr<AST> tree)
    {
        if (tree == nullptr)
            return 0;
        return visit(tree);
...
int main()
{
    ...
    Lexer lexer;
    //lexer.Run("3 ^ 2 ^ ");
     lexer.Load("/media/djoker/data/code/projectos/compilers/Lets_Build_A_Interpreter/6_power_operator/main.v");
    ...
    try
    {
         if (lexer.Compile())
        {
            ...
            {                    
                ...
                double result = interpreter.Execute(tree);
                cout  << endl;
